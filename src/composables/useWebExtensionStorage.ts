 ...
 * @param options
 */
export function useWebExtensionStorage<T>(key: string, initialValue: MaybeRefOrGetter<T>, options: WebExtensionStorageOptions<T> = {}): RemovableRef<T> {
	const {
		flush = 'pre',
		deep = true,
	...
	async function read(event?: { key: string; newValue: string | null }) {
		...
		try {
			...
			if (rawValue == null) {
				...
				else if (type === 'object' && !Array.isArray(value)) data.value = { ...(rawInit as Record<keyof unknown, unknown>), ...(value as Record<keyof unknown, unknown>) } as T
